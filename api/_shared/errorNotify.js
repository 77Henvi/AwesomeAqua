...
const { sendLineNotify } = require('./lineNotify.js');

const {
  DISCORD_ERROR_WEBHOOK_URL: DISCORD_URL,
} = process.env;

// ── ตัดข้อความยาวเกิน ไม่ให้ Discord ปฏิเสธ request (Discord limit ~2000 ตัวอักษร) ──
...
async function notifyError(source, error) {
  ...
  await Promise.all([
    ...
    sendLineNotify(`🚨 [AwesomeAqua Error] ${source}\n${message}`),
  ]);
}
