# ตั้งค่า Error Monitoring + Line แจ้งเตือนสำรอง
...
อัตโนมัติ แทนที่จะเงียบหายไปจน backend พังโดยไม่มีใครรู้ และ `api/notify-low-stock.js`
ก็ใช้ตัวแปรเดียวกันนี้เป็น **ช่องทางสำรอง** เวลาแจ้งเตือนสต็อกใกล้หมด/หมด
...
- `LINE_NOTIFY_TOKEN` ตัวเดียวกันนี้ยังถูกใช้เป็น **ช่องทางสำรอง** สำหรับแจ้งเตือนสต็อกใกล้หมด
  (`api/notify-low-stock.js`) ด้วย — ถ้า Messenger ส่งไม่ถึงแอดมิน (block บอทเผลอ/token หมดอายุ)
  แอดมินยังได้รับแจ้งเตือนทาง Line อยู่ ไม่ต้องตั้งค่าเพิ่มอะไรอีก ใช้ตัวแปรเดียวกัน
